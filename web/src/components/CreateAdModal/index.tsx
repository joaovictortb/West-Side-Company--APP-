...
export const CreateAdModal: React.FC = () => {
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content className="border-t-4 border-l-4 border-opacity-50 border-indigo-500  fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <div className="mt-2 flex items-center gap-2">
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                ...
                >
                  <Checkbox.Indicator>
                    <Check className="aspect-square w-4 text-green-600" />
                  </Checkbox.Indicator>
                </Checkbox.Root>
              )}
            />
          ...
          <footer className="mt-4 flex justify-end gap-4">
            <Dialog.Close
              type="button"
              className="rounded-md bg-zinc-500 py-2.5 px-3.5 text-sm font-semibold transition-colors hover:bg-red-600 sm:px-5 sm:py-3 sm:text-base"
            >
              Cancelar
            </Dialog.Close>
            ...
            <button
              type="submit"
              className="flex items-center gap-3 rounded-md bg-violet-500 py-2.5 px-3.5 text-sm font-semibold transition-colors hover:bg-violet-700 sm:py-3 sm:px-5 sm:text-base"
            >
